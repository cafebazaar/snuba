...
class SubscriptionScheduler(Scheduler[Subscription]):
    def __init__(
        ...
        self.__metrics = MetricsWrapper(
            metrics, "schedule", tags={"partition": str(self.__partition_id)}
        )
    ...
    def __get_subscriptions(self) -> List[Subscription]:
        ...
        if (
            ...
            self.__metrics.gauge("size", len(self.__subscriptions))

        self.__metrics.timing(
            "staleness", (current_time - self.__last_refresh).total_seconds() * 1000.0
        )
